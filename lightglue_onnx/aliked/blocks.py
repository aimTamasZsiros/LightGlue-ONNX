...
class DeformableConv2d(nn.Module):
    ...
    def forward(self, x):
        ...
        # Convert dynamic shape dimensions to tensors so we can use torch.max
        # Ensure they are float for division and on the same device as the input tensor
        h_tensor = torch.tensor(h, device=x.device, dtype=torch.float32)
        w_tensor = torch.tensor(w, device=x.device, dtype=torch.float32)

        # Use torch.max to create a dynamic operator in the ONNX graph
        max_dim = torch.max(h_tensor, w_tensor)
        max_offset = max_dim / 4.0
